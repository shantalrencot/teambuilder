...
-- Must be created BEFORE the RLS policies that reference them
-- ------------------------------------------------------------
...
RETURNS user_role LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  ORDER BY CASE role
    ...
    WHEN 'executive'   THEN 2 WHEN 'management' THEN 3
    WHEN 'sme'         THEN 4 WHEN 'high_grade' THEN 5
    WHEN 'mid_grade'   THEN 6 WHEN 'low_grade'  THEN 7
    WHEN 'youth'       THEN 8 ELSE 9
  END
  LIMIT 1;
$$;

-- ------------------------------------------------------------
-- RLS POLICIES (functions exist now, safe to reference them)
-- ------------------------------------------------------------
...
CREATE POLICY "super_admin write" ON public.user_roles FOR ALL
  USING (public.has_role(auth.uid(), 'super_admin'))
  WITH CHECK (public.has_role(auth.uid(), 'super_admin'));

ALTER TABLE public.courses ENABLE ROW LEVEL SECURITY;
CREATE POLICY "public read published" ON public.courses FOR SELECT USING (is_published = true);
CREATE POLICY "admin write all" ON public.courses FOR ALL
...
CREATE POLICY "owner update" ON public.enrollments FOR UPDATE USING (auth.uid() = user_id);
...
CREATE POLICY "owner insert"   ON public.leaderboard_points FOR INSERT WITH CHECK (auth.uid() = user_id);
